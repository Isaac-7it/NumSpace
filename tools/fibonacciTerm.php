...
if($_SERVER['REQUEST_METHOD'] === 'GET' && array_key_exists('number', $_GET)) {
    ...
    if(filter_var($inputValue, FILTER_VALIDATE_INT)) {
        ...
        if($inputValue <= 0) {
            $error .= 'Error! We expect a natural number.';
        } else {
            $result = (new Fibonacci()) -> getFibonacciTerm($inputValue);
        }
...
?>
    <div class="custom-grid mb-4">
        ...
        <a href="./fibonacciTerm.php" class="flex items-center justify-center rounded-[10px] active-bg active-text p-2">Fibonacci Term Generator</a>
    ...
    <form class="" method="GET">
        <div class="mb-6">
            <h2 class="text-[24px] font-medium text-[#333333]">Fibonacci Term Generator</h2>
        ...
        <div class="">
            ...
            <button type="submit" class="bg-[#4F39F6] text-white w-full rounded-lg p-2">Obtain nth Term</button>
        </div>
    </form>
    </div>
    <span class="bg-[#F3F4F6] block h-0.5 mb-4">&nbsp;</span>
